...
int readInput(char* inputFile)
{
  if (inputFormat == 'a')
  {
    ...
    // Read in nvt, nvb, nh
    fscanf(inputStream,"%d",&nvt);
    fscanf(inputStream,"%d",&nvb);
    fscanf(inputStream,"%d",&nh);
    nm = nvt + nvb + nh;
    fgetc(inputStream);
